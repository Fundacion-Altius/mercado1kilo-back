...
#     """
#     Lookup product details using EAN code with database fallback
#     """
...
#         if product:
#             # Format response according to your model structure
#             result = {
#                 'ean': product.ean,
#                 'name': product.name,
#                 'category': {
#                     'id': product.category.id,  # Assuming ForeignKey to Category
...
#         # Save to database (adjust according to your model structure)
#         category, _ = Category.objects.get_or_create(
#             id=product_data.get('categoryId'),
#             defaults={'name': product_data.get('categoryName')}
#         )
...
def product_list(request):
    ...
    ean = request.query_params.get('ean')
    if not ean:
        return Response({"error": "EAN parameter is required"}, status=status.HTTP_400_BAD_REQUEST)
